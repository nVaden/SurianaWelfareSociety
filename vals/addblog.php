...
// $internalerrmsg = '';
    ...
    //                       if(move_uploaded_file($temp, $tmp_file_path)){
                            $getcategory = mysqli_fetch_array(mysqli_query($connecDB, "SELECT category FROM blogcategory WHERE id = $category"));
                            $categoryname = $getcategory['category'];
                            $addimgQuery = mysqli_query($connecDB, "INSERT INTO blog(url, title, category, categoryid, shortinfo, fullpost, status, postdate)
                              VALUES('$posturl', '$title', '$categoryname', '$category', '$shortinfo','$fullinfo', 1, now())");
                            ...
                            if($addimgQuery){
                              ...
                              $_SESSION['rmnotfymsg'] = '<p class="alert alert-success">Post is added successfully.</p>';
                              header('location: '.$defaulturl['home'].'/blog.php');
